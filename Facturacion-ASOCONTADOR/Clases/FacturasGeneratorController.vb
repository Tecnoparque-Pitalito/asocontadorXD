...
Namespace Clases
    Friend Class FacturasGeneratorController
        ...
        Friend Function recorerPersonas(telefono As Long) As Boolean
            Try
                ...
                pdfFormtoHorizontal(listaDePersonas, telefono) ' Pasa la lista de personas al método
                listaDePersonas = New List(Of Persona)()
                Return True
            Catch ex As Exception
        ...
        Private Sub pdfFormtoHorizontal(listaDePersonas As List(Of Persona), telefono As Long)
            Try
                If listaDePersonas Is Nothing OrElse listaDePersonas.Count = 0 Then
                ...
                For Each persona As Persona In listaDePersonas
                    ...
                    Dim invoiceTable As PdfPTable = CreateInvoice(persona, telefono)
        ...
        Private Function CreateFooterTable(persona As Persona) As PdfPTable
            ...
            Dim boldFont As Font = FontFactory.GetFont(FontFactory.HELVETICA_BOLD, 8)
            Dim titleFont As Font = FontFactory.GetFont(FontFactory.HELVETICA_BOLD, 10, BaseColor.BLACK)

            ' Tabla para la colilla de pago
            Dim footerTable As New PdfPTable(3)
            footerTable.WidthPercentage = 100
            footerTable.SetWidths({15, 55, 30})
        ...
        Private Function CreateInvoice(persona As Persona, telefono As Long) As PdfPTable
            ' Configuración de fuentes
            ...
            Dim titleFont As Font = FontFactory.GetFont(FontFactory.HELVETICA_BOLD, 11, BaseColor.BLACK)
            ...
            clientInfoTable.AddCell(New PdfPCell(New Phrase($"{persona.NombreCliente}", titleFont)) With {.Border = Rectangle.BOX})
            ...
            clientInfoTable.AddCell(New PdfPCell(New Phrase("Dirección:", boldFont)) With {.Border = Rectangle.BOX})
            clientInfoTable.AddCell(New PdfPCell(New Phrase($"{persona.Direccion}", titleFont)) With {.Border = Rectangle.BOX})
            clientInfoTable.AddCell(New PdfPCell(New Phrase("Fontanero:", boldFont)) With {.Border = Rectangle.BOX})
            clientInfoTable.AddCell(New PdfPCell(New Phrase($"{telefono.ToString()}", regularFont)) With {.Border = Rectangle.BOX})
            clientInfoTable.AddCell(New PdfPCell(New Phrase("Pagos:", boldFont)) With {.Border = Rectangle.BOX})
            clientInfoTable.AddCell(New PdfPCell(New Phrase($"Cuenta de Ahorros de Bancolombia N° 453868061-48", titleFont)) With {.Border = Rectangle.BOX})
    ...
    Public Class Persona
        ...
        'constructor
        Public Sub New(IdFactura As String, NombreCliente As String, direccion As String, NombrePredio As String, TelefonoCliente As String, PeriodoCobro As String, CostoPeriodo As String, TotalPagar As String, Reconexion As String, Multas As String, otros As String, atrasos As String, RecargoMora As String, AjustePeso As String, avisos As String, FechaPago As String)
