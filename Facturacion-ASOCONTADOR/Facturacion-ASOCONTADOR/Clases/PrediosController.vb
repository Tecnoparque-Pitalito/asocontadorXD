...
Namespace Clases
    Friend Class PrediosController
        ...
        Friend Function BuscarPredios(txtNumeroCedula As TextBox, dgvPredios As DataGridView) As Integer
            Try
                ...
                Using conn As New SQLiteConnection(connectionString)
                    ...
                    Dim queryPredios As String = "SELECT idPredios AS codigo, 
                                                  Vereda, 
                                                  CASE 
                                                  ...
                                                  Observaciones, 
                                                  NumeroMatricula,  
                                                  NumeroRegistroCatastral, 
                                                  Usuarios_NumeroCedula 
                                           FROM predios 
                            ...
                            Else
                                ...
                                Using cmd As New SQLiteCommand(queryPeriodo, connection)
                                    ...
                                    Using reader As SQLiteDataReader = cmd.ExecuteReader
                                        If reader.Read Then
                                            If Not IsDBNull(reader("FinPeriodo")) Then
                                                Dim finPeriodo As DateTime = DateTime.Parse(reader("FinPeriodo").ToString)
                                                Dim inicio As DateTime = DateTime.Now
                                                ...
                                                While inicio <= finPeriodo
                                                    ...
                                                    Dim queryUpdateTrue As String = "UPDATE factura 
                                                                              SET estado = 3 
                                                                              WHERE estado = 4 AND Predios_idPredios = @IdPredio AND mes = @mes AND Periodos_idPeriodos = @anyo"
                                                    Using cmdUpdate As New SQLiteCommand(queryUpdateTrue, connection, transaction)
                                                        ...
                                                        cmdUpdate.Parameters.AddWithValue("@anyo", anyo)
        ...
        Friend Sub listarIDNombre(comboBox1 As ComboBox, textBox10 As TextBox)
            Try
                Using conn As New SQLiteConnection(connectionString)
                    conn.Open()
                    Using cmd As New SQLiteCommand("SELECT pre.idPredios, pre.NombrePredio FROM predios pre WHERE Usuarios_NumeroCedula = @cedula", conn)
